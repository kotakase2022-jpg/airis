...
async function main() {
  ...
  for (const [loginId, role, name, agencyId, status, deletedAt] of lifecycleAccounts) {
    await prisma.account.upsert({
      where: { loginId }, update: {},
      create: {
        loginId, role, name, agencyId, status,
  ...
  if (staff1 && staff2) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      await prisma.dailyReport.upsert({
        ...
        update: {},
        create: {
          date, type: "テレマ", salesStaffId: staff2.id, agencyId: staff2.agencyId,
          ...
          activityContent: "既存リストへの架電", activityResult: "アポ獲得あり",
        },
      });
    }
  }
  ...
  for (const [series, no, tpl, status, deadline, primaryId] of extraCases) {
    ...
    const agencyName = primaryId === p1.id ? "東都ネットワーク販売株式会社" : "関西モバイルサポート株式会社";
    const c = await mkCase(series, no, tpl, `${tpl}／${agencyName}／9999999999`, status, deadline, primaryId);
  ...
  const faStaff = await prisma.salesStaff.findMany({
    where: { salesId: { not: null }, status: { in: ["registered", "provisional"] }, agency: { isDummy: false } },
    include: { agency: true },
    orderBy: { salesId: "asc" },
    take: 10,
  });
  ...
  if (annCount === 0) {
    ...
    await prisma.announcement.create({
      data: {
        audience: "all", title: "販売員IDの一括申請CSVひな形を更新しました",
    ...
    await prisma.announcement.create({
      data: {
        audience: "primary", title: "【1次店向け】訪販員申請の受付スケジュール",
  ...
  for (const [i, st] of dummyStaff.entries()) {
    ...
    for (let d = 1; d <= 5; d++) {
      ...
      await prisma.dailyReport.upsert({
        ...
        update: {},
        create: {
          date, type: "訪販", salesStaffId: st.id, agencyId: st.agencyId,
          area: "サンプル区", forecastAcq: 10, acquisitions: d % 3, workers: 1,
          visits: 30 + d, meetings: 10 + d, negotiations: 5 + d, contracts: d % 2,
          activityContent: "（架空データ）サンプル巡回", activityResult: "（架空データ）反応良好",
        },
      });
    }
  }
  ...
  if (dummyStaff.length > 0) {
    for (const kind of SUB_KINDS.slice(0, 4)) {
      if (await prisma.submission.findFirst({ where: { kind, targetMonth: month, submitterAgencyId: d1.id } })) continue;
      const content = `（架空データ）${kind} サンプル一次代理店株式会社 ${month}`;
      const stored = await prisma.storedFile.create({
        data: { name: `SAMPLE_${kind}_${month}.txt`, mime: "text/plain", size: Buffer.byteLength(content), data: Buffer.from(content), uploadedBy: "seed" },
      });
      await prisma.submission.create({
        data: {
          kind, fiscalYear: Number(month.slice(0, 4)), targetMonth: month,
          primaryAgencyId: d1.id, submitterAgencyId: d1.id,
          fileId: stored.id, fileName: stored.name,
          status: ["pending_first", "pending_snc", "approved", "rejected"][SUB_KINDS.indexOf(kind) % 4],
          history: [{ event: "submitted", at: `${month}-05`, by: "seed" }],
        },
      });
    }
  ...
  for (const [i, ag] of [d1, d2].entries()) {
    ...
    await prisma.account.upsert({
      where: { loginId }, update: {},
      create: {
        loginId, role: ag.tier === 1 ? "R7" : "R8",
        name: `見本 ${ag.tier === 1 ? "一次" : "二次"}管理者`,
        agencyId: ag.id, status: "active",
        passwordHash: hash(PASSWORDS.general), mustChangePassword: false,
        email: `${loginId}@example.com`,
      },
    });
  }
