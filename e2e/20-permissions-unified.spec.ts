...
const CSV_MATRIX: { path: string; allowed: RoleKey[]; spec: string }[] = [
  ...
  },
  {
    ...
    // ③は管理画面には入れる（発注者指示 2026-08-05「③の管理画面を〇」）が、
    // **監査記録（棚卸CSV・監査ログ・アクセスログ）には到達不可**（発注者指示 2026-08-06）。
    // ④はダミー表示のため実データのエクスポート不可（§3.5）。
    allowed: ["R1", "R2"],
    spec: "§7.2 棚卸CSV=①②のみ（③は発注者指示2026-08-06で到達不可。④ダミーは実データのエクスポート不可 §3.5）",
  },
  {
    path: "/admin/csv?type=audit",
    allowed: ["R1", "R2"],
    spec: "§7.1 監査ログ=①②のみ（③は発注者指示2026-08-06で到達不可）",
  },
  {
    path: "/admin/csv?type=access",
    allowed: ["R1", "R2"],
    spec: "§7.2 アクセスログCSV（IP/UA付き）=①②のみ（③は発注者指示2026-08-06で到達不可）",
  },
];
