...
## BUG-L20（重大・本番事故）: 本番検証テストが本番アカウントのMFAを本登録し、利用者がログイン不能になった

- **検出**: 発注者からの申告（①`airis_slb_sys_001` でMFAを通過できない）
- **事象**: `e2e-prod` のログイン処理が、MFA未登録アカウントで `/mfa/setup`（初回登録画面）に
  到達すると、**サーバが発行した秘密鍵をDBから読み、そのまま「登録して続行」を押していた**。
  その秘密鍵はDBにしか存在せず利用者の認証アプリには無いため、**本人がログインできなくなる**。
- **確定した証跡**（監査ログ `mfa_enroll` の実行元IP）:

  | 日時(UTC) | アカウント | 実行元IP | |
  |---|---|---|---|
  | 2026-08-05 04:06:12 | airis_slb_sys_001 | 124.35.91.98 | 利用者本人の登録 |
  | 2026-08-05 12:11:42 | airis_slb_sys_001 | 150.249.201.199 | **検証実行が上書き** |
  | 2026-08-05 12:12:08 | airis_2210001_001 | 150.249.201.199 | 同上 |
  | 2026-08-05 12:12:11 | airis_1190001_001 | 150.249.201.199 | 同上 |

- **一次対応**: 発注者が全アカウントのMFAを一括リセット（`actor=system-maintenance` の
  `mfa_reset` 監査記録41件を確認）。①は本人が再登録済み（08-08 02:15、IP 124.35.91.98）。
- **二次事故（修正漏れ）**: `prod-smoke.spec.ts` だけを修正して本番検証を実行したところ、
  **同じ処理の複製が `prod-authz-verify.spec.ts` に残っており**、
  ②`airis_snc_adm_001` と ③`airis_snc_ops_0001` を新たに登録してしまった
  （08-08 02:22:07 / 02:22:16、IP 150.249.201.199）。
  **同じ処理が2箇所に複製されていたことが、事故を二段構えにした。**
- **恒久対処**:
  1. `e2e-prod/helpers.ts` を新設し、接続（`db()`）・ログイン・MFA通過を**1箇所に集約**。
     両specから複製を削除した。
  2. `passMfaOrFail()` は `/mfa/setup` に到達したら**何も入力せず例外で失敗**させる
     （前提不成立）。押すボタンは「認証する」のみで、「登録して続行」を押す経路を消した。
     さらに `mfaEnabled === true` を明示的に検査する。
  3. `pickLoginId()` は**MFA登録済みのアカウントを優先**して選ぶ。
- **再発防止テスト**: `tests/unit/prod-e2e-safety.test.ts`（6件）
  - どの spec も「登録して続行」を含まないこと
  - helpers が押すのは「認証する」のみであること
  - `/mfa/setup` 到達時に `throw` する実装があること
  - **spec に `new PrismaClient(` / `generateSync(` の複製が無く、helpers を import していること**
    （複製の再発自体を検出する）
- **検証（実測）**: 修正後に本番検証を実行し、実行前後で

  ```
  mfaEnabled=3 / secret保持=9 / mfa_enroll累計=128   （実行前）
  mfaEnabled=3 / secret保持=9 / mfa_enroll累計=128   （実行後・完全に不変）
  ```

  であることを確認した。MFA必須ロールのうち登録済みアカウントが無いものは
